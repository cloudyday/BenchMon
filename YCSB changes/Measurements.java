...
public class Measurements
{
	...
	HashMap<String,OneMeasurement> dataHistogram;
	HashMap<String,OneMeasurement> dataTimeseries;
	...
	{
		dataHistogram=new HashMap<String,OneMeasurement>();
		dataTimeseries=new HashMap<String,OneMeasurement>();
	...
	OneMeasurement constructOneMeasurement(String name, boolean histogram)
	{
		if (histogram)
		{
	...
	{
		if (!dataHistogram.containsKey(operation))
		{
			synchronized(this)
			{
				if (!dataHistogram.containsKey(operation))
				{
					dataHistogram.put(operation,constructOneMeasurement(operation, true));
					dataTimeseries.put("ts_"+operation,constructOneMeasurement("ts_"+operation, false));
				}
			}
		}
		try
		{
			dataHistogram.get(operation).measure(latency);
			dataTimeseries.get("ts_"+operation).measure(latency);
		}
	...
	public void reportReturnCode(String operation, int code)
	{
		...
		dataHistogram.get(operation).reportReturnCode(code);
		dataTimeseries.get("ts_"+operation).reportReturnCode(code);
	}
  ...
  {
    for (OneMeasurement measurement : dataHistogram.values())
    {
      measurement.exportMeasurements(exporter);
    }
    for (OneMeasurement measurement : dataTimeseries.values())
    {
      measurement.exportMeasurements(exporter);
    }
  }
	...
	{
		String ret="";
		for (OneMeasurement m : dataHistogram.values())
		{
			ret+=m.getSummary()+" ";
		}
